...
- 每个插件消息段可标记为「招呼」或「补充」，并可用总开关关闭插件的全部文字消息。
- BOSS 自动招呼开启且平台确认已发送时，插件跳过所有「招呼」段，仅发送「补充」段；关闭时按顺序发送两类消息。
...
- 每一段可以单独启用或删除。
- 每一段都要选择角色：「招呼」用于首次问候，「补充」用于项目经历、简历说明等后续内容。
- 发送顺序就是页面中的段落顺序。
...
插件不会自动点击 BOSS 聊天页的“发简历”按钮。该按钮通常需要招聘者先回复、形成双向沟通后才可用；自动投递阶段只支持发送用户配置的图片简历。
...
扫描预览会持续把 BOSS 动态职位列表滚动到底，并按岗位标识累计去重；如果 60 秒内仍未到底，就停止滚动并使用当时累计的全部岗位开始筛选。滚动阶段不根据通过率或投递上限提前停止，面板会实时显示累计岗位数，并在结果中写明滚动轮数、耗时和停止原因。扫描只读取和筛选岗位，不会发起沟通。

职位名称、公司名称、JD、公司黑白名单统一使用筛选专用标准化：忽略英文大小写、全角/半角、空格、换行、连字符、下划线和常见中英文标点。因此 `AI Agent`、`aiagent`、`AI-Agent` 会按同一词匹配；`C++`、`C#` 中有意义的符号仍会保留。

筛选和消息仍会自动保存，但消息输入采用 1.8 秒空闲防抖，并识别中文输入法组合态；拼音尚未上屏时不会保存、刷新或重建输入框，也不会弹出保存提示。
...
BOSS 通常要求招聘者先回复、形成双向沟通后才开放“发简历”。从 v1.7.11 起插件不再自动点击该按钮，避免把平台门禁误判成投递失败；如需自动随消息发送简历，请配置图片简历。
