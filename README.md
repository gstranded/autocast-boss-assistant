...
扫描预览会自适应向下加载 BOSS 的动态职位列表并累计去重：默认至少观察 45 岗，通过率较低时会继续多滚动；符合岗位达到“本次最多沟通”、列表确实到底、累计 300 岗或总耗时达到 50 秒时停止。面板会实时显示已扫描/通过数量，并在结果中写明滚动轮数、耗时和停止原因。扫描只读取和筛选岗位，不会发起沟通。
