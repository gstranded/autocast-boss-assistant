...
- 扫描预览会在不激活的临时职位标签页中滚动 BOSS 动态列表，原职位页、筛选、详情和滚动位置保持不动；如果 60 秒内仍未到底，就使用当时累计的岗位开始筛选，临时页随后自动关闭。面板每秒显示计时和累计岗位数（v1.7.14）。
- 扫描批次只传输本批新增岗位；扫描期间使用轻量状态同步，不重复读取完整任务、历史、简历和日志。隐藏面板会暂停轮询，面板打开状态也不再跨 BOSS 标签共享（v1.7.14）。
- 扫描的 60 秒上限只控制滚动采集，到时会正常筛选已收集岗位；页面操作和后台结果通道共享同一时间预算，取消、页面超时与通道失联分别收口。临时扫描页失败时不会回退滚动原职位页（v1.7.14）。
- 扫描只读取和筛选岗位，不会发起沟通。
