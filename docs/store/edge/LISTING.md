...
包：`dist/autocast-boss-haitou-v1.7.15.zip`
...
```text
更新现有扩展，不是新产品。版本 1.7.15。

变化：
- 扫描改为直接滚动当前职位列表，完整保留 BOSS SPA 中的求职期望、城市和筛选上下文
- 保留当前可见岗位后回到列表顶部，快速触达当前底部并约每 300 毫秒确认懒加载结果；确认到底或达到 60 秒后才统一筛选一次
- HR 活跃筛选不再把列表卡缺失的状态误报为“未知且不满足”；投递前在临时岗位详情页只读校验，通过后才会点击沟通
- 修复隐藏临时页丢失“算法工程师(深圳)”并实际扫描“推荐 + 广州”，导致数量卡在 15 岗的问题
- 60 秒改为采集硬截止，不再叠加页面桥接和取消等待；调试数据仍保留滚动元素、初始/最终高度和增长次数
- 扫描界面只显示当前阶段和累计岗位数，不再展示秒数、轮数及内部停止原因

测试：
1. 打开 https://www.zhipin.com/web/geek/jobs 并登录
2. 刷新后点右侧悬浮按钮
3. 扫描预览，确认当前职位页快速向下加载，累计岗位数随懒加载增长
4. 等待到底或达到 60 秒，确认预览总数等于累计去重岗位，并显示通过/排除原因
5. 可用「投递一份」验证发送
