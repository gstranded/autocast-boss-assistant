...
包：`dist/autocast-boss-haitou-v1.7.16.zip`
...
```text
更新现有扩展，不是新产品。版本 1.7.16。

变化：
- 扫描改为约 85% 视口的快速重叠步进，约每 100 毫秒读取一个窗口；既覆盖虚拟列表中间岗位，也能持续触发 BOSS 懒加载
- 到达当前底部后自动回拉再触底，并等待列表稳定；修复页面实际已有 90+ 岗位但预览仍停在 60 岗的问题
- 恢复扫描逐秒计时；60 秒预算内为结果序列化和回传预留独立收尾时间，不再因截止点竞态丢弃最后一批
- 扫描同步复用 BOSS 自己的列表响应读取在线状态、securityId 和 lid；仅对其他条件已通过的少量候选限流读取详情活跃度
- HR 详情预检遇到任何非零响应或风控状态会立即熔断且不重试；无法预检的岗位仍在正式沟通前强校验
- 页面只显示“加载岗位 / 核对 HR 状态 / 筛选岗位”和秒数，轮数、内部截止原因等仍只保留在调试数据

测试：
1. 打开 https://www.zhipin.com/web/geek/jobs 并登录
2. 刷新后点右侧悬浮按钮
3. 扫描预览，确认当前职位页连续快速向下加载，状态每秒显示计时
4. 等待到底或达到 60 秒，确认预览总数等于累计去重岗位，并显示通过/排除原因；启用 HR 活跃筛选时会出现“核对 HR 状态”阶段
5. 可用「投递一份」验证发送
