...
公开同类插件普遍已具备自动批量沟通、关键词/地区/薪资/HR 活跃筛选、本地防重复、任务面板、日志与 AI 回复等能力。仅实现“多段自我介绍 + 图片/附件简历”不足以形成差异。
...
- **两个独立开关**，不是单一总开关。
- 默认时机建议：文本发送完成后，**仅在 HR 请求简历时**发送；“首次沟通后立即发送”为可选项，且不建议默认同时开图片+附件。
- 图片：多页顺序串行发送，格式/大小校验，失败可重试，防重复。
- 附件：PDF/DOC/DOCX，显示名称与版本时间，发送前确认当前文件。
