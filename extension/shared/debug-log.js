...
function cleanString(value, maxLength = 2400) {
...
export function sanitizeDebugValue(value, depth = 0, seen = new WeakSet()) {
  ...
  if (Array.isArray(value)) {
    const rows = value.slice(0, 60).map((item) => sanitizeDebugValue(item, depth + 1, seen));
...
export function appendSessionDebugLog(entry) {
  writeChain = writeChain.then(async () => {
    ...
    const next = trimToBudget([...current, sanitizeDebugValue(entry)]);
